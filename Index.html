...
<div id="app">
<div class="header">
  <div><span class="logo">⚡ MEMECOIN</span><span class="logo-sub">SNIPER v4.1 ⚡</span></div>
  <div class="header-status" id="headerStatus">No wallet loaded</div>
</div>
<div class="banner banner-err" id="errBanner"></div>
<div class="banner banner-warn show">⚠️ This bot trades real SOL. Only use a dedicated wallet with small funds you can afford to lose.</div>
<div class="key-section">
  <div class="key-label">Wallet Private Key</div>
  <div class="key-row">
    <input type="password" class="key-input" id="keyInput" placeholder="Paste your base58 private key…"/>
    <button class="btn-key" id="btnKey" onclick="loadKey()">Load</button>
  </div>
  <div class="key-hint">⚡ Scans DexScreener trending tokens for recent large buys. Updates every 5 seconds.</div>
</div>
<div class="controls">
  <button class="btn-main btn-start" id="btnBot" onclick="toggleBot()" disabled>▶  START BOT</button>
  <div class="stats-row">
...
<div class="page active" id="page-dashboard">
  ...
  <div class="panel"><div class="panel-head">Active Tokens (5min volume)</div><div id="scanList"><div class="empty">Waiting for scan…</div></div></div>
</div>
<div class="page" id="page-settings">
  <div id="settingsContainer"></div>
  <button class="btn-sm" onclick="resetSettings()" style="width:100%;padding:10px;margin-top:4px;">↺ Reset to defaults</button>
</div>
...
const LAMPORTS=1e9;
const SOL_PRICE=200; // rough estimate for USD calculations

const DEFAULTS={buyAmountSol:0.05,takeProfitPct:3,maxHoldSecs:30,minBuyUsd:1000,maxMarketCapUsd:500000,maxConcurrent:2,slippageBps:200};
const SETTING_DEFS=[
  ...
  {key:"minBuyUsd",        label:"Min Buy Size (USD)",     step:100,   min:100},
  {key:"maxMarketCapUsd",  label:"Max Market Cap (USD)",   step:50000, min:10000},
...
let rpcUrl="https://mainnet.helius-rpc.com/?api-key=cdad2726-4c49-47ad-9d58-93c2e4eed7fe", connection=null, balance=null;
...
async function loadKey(){
  ...
  try{
    ...
    document.getElementById('headerStatus').innerHTML=`<span class="bal">${balance.toFixed(4)} SOL</span><br/>${pubkey.slice(0,8)}…${pubkey.slice(-4)}`;
    document.getElementById('btnBot').disabled=false;
    log(`Wallet: ${pubkey.slice(0,8)}…${pubkey.slice(-4)} | ${balance.toFixed(4)} SOL`,'success');
    if(balance<settings.buyAmountSol*1.5)log('⚠️ Low balance','warn');
  }catch(e){keypair=null;pubkey=null;showErr('Invalid key: '+(e.message||e));log('Key error: '+(e.message||e),'error');}
}

// ════════════════════════════════════════════════════════════════
// SIGN & SEND — with detailed error logging
// ════════════════════════════════════════════════════════════════
async function signAndSend(base64Tx){
  try{
    const raw=Uint8Array.from(atob(base64Tx),c=>c.charCodeAt(0));
    log(`TX bytes: ${raw.length}, first byte: 0x${raw[0].toString(16)}`);
    
    const isVersioned=(raw[0]&0x80)!==0;
    log(`TX type: ${isVersioned?'versioned':'legacy'}`);
    
    if(isVersioned){
      log('Deserializing as versioned TX...');
      const vTx=solanaWeb3.VersionedTransaction.deserialize(raw);
      log('Signing versioned TX...');
      vTx.sign([keypair]);
      log('Sending versioned TX...');
      return await connection.sendRawTransaction(vTx.serialize(),{skipPreflight:false,preflightCommitment:'finalized'});
    } else {
      log('Deserializing as legacy TX...');
      const tx=solanaWeb3.Transaction.from(raw);
      log('Signing legacy TX...');
      tx.sign(keypair);
      log('Sending legacy TX...');
      return await connection.sendRawTransaction(tx.serialize(),{skipPreflight:false,preflightCommitment:'finalized'});
    }
  }catch(e){
    log('signAndSend error: '+(e.message||e),'error');
...
// ════════════════════════════════════════════════════════════════
// SCAN — DexScreener for tokens + check for large recent buys
// Uses DexScreener trending + filters for actual large transactions
// ════════════════════════════════════════════════════════════════
async function scanForLargeBuys(){
  try{
    log('Scanning for tokens with large buys...');
    
    // Get trending tokens from DexScreener
    const br=await fetch("https://api.dexscreener.com/token-boosts/top/v1");
    if(!br.ok){log('DexScreener '+br.status,'error');return[];}
    ...
    for(const b of(boosts||[])){
      if(b.chainId==='solana'&&b.tokenAddress){
        ...
        if(mints.length>=30)break; // check top 30
      }
    }
    if(!mints.length){log('No trending tokens','warn');return[];}
    log(`Checking ${mints.length} trending tokens for large buys...`);
    ...
    for(const mint of mints){
      ...
      try{
        ...
        const v5=pair.volume?.m5||0;
        const price=parseFloat(pair.priceUsd||0);
        const priceChange5m=parseFloat(pair.priceChange?.m5||0);
        const buyCount=pair.txns?.m5?.buys||0;
        
        // Filter: low liquidity
        if(liq>settings.maxMarketCapUsd||liq<10000)continue;
        
        // Filter: has 5min volume
        if(v5<settings.minBuyUsd)continue;
        
        // Filter: positive price action
        if(priceChange5m<1)continue;
        
        // Filter: has recent buys
        if(buyCount<1)continue;
        
        // Calculate average buy size
        const avgBuy=buyCount>0 ? v5/buyCount : 0;
    ...
    // Sort by highest 5min volume
    out.sort((a,b)=>b.volume5min-a.volume5min);
    log(`Found ${out.length} tokens with recent activity`);
    return out;
    
  }catch(e){log('Scan error: '+(e.message||e),'error');return[];}
}
...
async function buyToken(c){
  const{mint,symbol,marketCap,avgBuy,buyCount,volume5min}=c;
  const amt=Math.floor(settings.buyAmountSol*LAMPORTS);
  log(`💎 BUY: ${symbol} | mcap $${marketCap.toFixed(0)} | vol5m $${volume5min.toFixed(0)} | avgBuy $${avgBuy.toFixed(0)}`);
  
  let q;
  try{
    q=await getQuote(SOL_MINT,mint,amt);
  }catch(e){
    log('✗ Quote fetch error: '+(e.message||e),'error');
    return;
  }
  
  if(!q){log('✗ No quote for '+symbol,'error');return;}
  const outAmt=parseInt(q.outAmount||0);
  if(!outAmt){log('✗ Zero output','error');return;}
  
  let sr;
  try{
    sr=await fetch(JUP_SWAP,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userPublicKey:pubkey,quoteResponse:q,prioritizationFeeLamports:'auto'})});
  }catch(e){
    log('✗ Swap API error: '+(e.message||e),'error');
    return;
  }
  
  if(!sr.ok){log('✗ Jupiter HTTP '+sr.status,'error');return;}
  
  let swapResp;
  try{
    swapResp=await sr.json();
  }catch(e){
    log('✗ Swap response parse error: '+(e.message||e),'error');
    return;
  }
  
  if(!swapResp.swapTransaction){
    log('✗ No swapTransaction in response','error');
    return;
  }
  
  let sig;
  try{
    sig=await signAndSend(swapResp.swapTransaction);
  }catch(e){
    log('✗ Sign/send error: '+(e.message||e),'error');
    return;
  }
  
  log('✓ Buy TX: '+sig.slice(0,44)+'…','success');
  seenTokens.add(mint);
  positions.push({id:Date.now(),mint,symbol,entryPriceUsd:0,buyTime:Date.now(),amountLamports:amt,tokenAmount:outAmt});
  refreshBalance();renderDashboard();
  log(`✓ Holding ${outAmt} ${symbol}`,'success');
}
...
async function sellToken(pos,reason){
  log(`🔄 Selling ${pos.symbol} | ${reason}`);
  const q=await getQuote(pos.mint,SOL_MINT,pos.tokenAmount);
  if(!q){log('✗ No sell quote','error');return false;}
  ...
  if(!sr.ok){log('✗ Sell '+sr.status,'error');return false;}
  const swapResp=await sr.json();
  if(!swapResp.swapTransaction){log('✗ No sell TX','error');return false;}
  let sig;
  try{sig=await signAndSend(swapResp.swapTransaction);}catch(e){log('✗ Sell: '+(e.message||e),'error');return false;}
  ...
  log('✓ Sell TX: '+sig.slice(0,44)+'…','success');
  log(`✓ PnL: ${pnlSol>=0?'+':''}${pnlSol.toFixed(6)} SOL (${pnlPct>=0?'+':''}${pnlPct.toFixed(2)}%)`,pnlPct>=0?'success':'warn');
...
async function monitorPositions(){
  for(const pos of[...positions]){
    ...
    log(`👀 ${pos.symbol} | ${pct>=0?'+':''}${pct.toFixed(2)}% | ${elapsed.toFixed(0)}s`);
    if(pct>=settings.takeProfitPct){await sellToken(pos,`profit ${pct.toFixed(2)}%`);return;}
  ...
  while(!stopFlag){
    try{
      ...
      if(positions.length<settings.maxConcurrent){
        const active=await scanForLargeBuys();
        lastCands=active;renderScanList();
        if(active.length){log('💎 '+active.length+' active tokens');await buyToken(active[0]);}
        else log('💎 No active tokens');
      }else log('⏳ At max positions ('+settings.maxConcurrent+')');
    }catch(e){log('Loop: '+(e.message||e),'error');}
    const end=Date.now()+5000; // scan every 5s
    while(Date.now()<end&&!stopFlag){
      await sleep(2000);
      if(positions.length&&!stopFlag){try{await monitorPositions();}catch{}}
    }
  }
...
// ════════════════════════════════════════════════════════════════
function toggleBot(){
  if(botRunning){stopFlag=true;botRunning=false;log('⏹ Stopped');renderBotBtn();return;}
  if(!keypair){showErr('Load your key first');return;}
  if(balance<settings.buyAmountSol*1.1){showErr('Insufficient balance');return;}
  stopFlag=false;botRunning=true;
  log('🚀 Started — scanning DexScreener every 5s','success');
  renderBotBtn();botLoop();
}
function renderBotBtn(){
  const b=document.getElementById('btnBot');
  b.textContent=botRunning?'⏹  STOP BOT':'▶  START BOT';
...
function renderOpenList(){
  ...
  el.innerHTML=positions.map(p=>{
    ...
    return`<div class="pos-card"><div class="pos-top"><span class="pos-sym">${esc(p.symbol)}</span><span class="pos-badge" style="background:rgba(240,192,64,0.15);color:var(--yellow)">${s}s</span></div><div class="pos-bot"><span class="pos-detail">${(p.amountLamports/LAMPORTS).toFixed(3)} SOL</span></div></div>`;
  }).join('');
}
...
function renderScanList(){
  ...
  if(!lastCands.length){el.innerHTML='<div class="empty">Waiting...</div>';return;}
  el.innerHTML=lastCands.slice(0,10).map(c=>
    `<div class="scan-row"><span class="scan-sym">${esc(c.symbol)}</span><span class="scan-spike" style="color:var(--yellow)">$${(c.volume5min/1000).toFixed(1)}k vol</span><span class="scan-liq">${c.buyCount} buys</span></div>`
  ).join('');
}
...
// INIT
// ════════════════════════════════════════════════════════════════
(function(){renderSettings();renderLog();log('Ready. Scans DexScreener for tokens with large recent buys.');})();
</script>
</body>
</html>
